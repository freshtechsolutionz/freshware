...
export default async function OpportunitiesPage() {
  ...
  const [accountsRes, contactsRes, oppsRes, usersRes] = await Promise.all([
  ...
  return (
    <main className="p-6 max-w-6xl mx-auto space-y-4">
      ...
      <OpportunitiesClient
        ...
        initialRows={oppsRes.data ?? []}
        lookupError={accountsRes.error?.message ?? contactsRes.error?.message ?? usersRes.error?.message ?? null}
